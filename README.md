# VerticalAgent —— 多租户 AI Agent 平台

> 当前迭代：**Stage 2** —— 会话持久化 + RAG 知识库
>
...
```
VerticalAgent/                        # 仓库根（当前迭代 = Stage 2）
├── main.py              # FastAPI 入口（1300 行）
...
├── static/
│   └── index.html       # 前端（侧栏加搜索 + 文档库 tab + 文件上传 + RAG 引用）
├── stage1/              # 历史快照：多 Provider + SSE + Circuit Breaker
└── .github/workflows/
    └── ci.yml
```
